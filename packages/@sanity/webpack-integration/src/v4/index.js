...
function getPartPlugin(options) {
  return new PartPlugin(options)
}

function getPlugins(options) {
  return [getPartPlugin(options), getEnvPlugin(options)]
}
...
function getPostcssPlugins(options) {
  return [
    postcssImport({resolve: getStyleResolver(options)}),
    postcssPresetEnv({
      // importFrom: [],
      stage: 3,
      features: {
