...
import {dev} from '@sanity/runtime'
...
export default async function devAction(args, context) {
  ...
  dev(serverOptions).subscribe({
    next: msg => {
      // "invalid" doesn't mean the bundle is invalid, but that it is *invalidated*,
      // in other words, it's recompiling
      if (msg.type === 'webpack/invalid') {
        output.clear()
        resetSpinner()
      }

      // Hold off on showing the spinner until compilation has started
      if (msg.type === 'webpack/compile') {
        resetSpinner()
      }

      // "done" event fires when Webpack has finished recompiling the bundle.
      // Whether or not you have warnings or errors, you will get this event.
      if (msg.type === 'webpack/done_with_success') {
        if (compileSpinner) {
          compileSpinner.succeed()
        }

        output.print(
          chalk.green(`Content Studio successfully compiled! Go to http://${httpHost}:${httpPort}`)
        )
      }

      if (msg.type === 'webpack/done_with_errors') {
        const {hasErrors, hasWarnings, errors, warnings} = msg.stats
    ...
    error: error => {
      gracefulDeath(httpHost, config, error)
    }
  })
}
