...
import {uniqueId, isEqual, debounce} from 'lodash'
...
interface PTSelection {
  anchor: EditorSelectionPoint
  focus: EditorSelectionPoint
}

// eslint-disable-next-line max-statements
export const PTInput = React.memo((props: PTInputProps) => {
  // console.log('PTInput')
  ...
  const selection = useUnique(usePortableTextEditorSelection())
  const ptFeatures = useMemo(() => getPortableTextFeatures(type), [type])
  ...
  useEffect(() => {
    ...
    // console.time('Respond to focusPath changes')

    const sameSelection = selection && isEqual(selection.focus.path, focusPath)

    if (sameSelection) {
      // console.timeEnd('Respond to focusPath changes')
      return
    }
  ...
  const objectEditDataRef = useRef<ObjectEditData>(objectEditData)
  useEffect(() => {
    objectEditDataRef.current = objectEditData
  }, [objectEditData])
  ...
  const updateFocusPathRef = useRef<((sel: PTSelection) => void) | null>(null)

  // Update the FormBuilder focusPath as we get a new selection from the editor
  // This will also set presence on that path
  useEffect(() => {
    const updateFocusPath = debounce((sel: PTSelection) => {
      if (!sel) return

      const currFocusPath = focusPathRef.current

      // If the focuspath is a annotation (markDef), don't update focusPath,
      // as this will close the editing interface
      const isAnnotationPath = currFocusPath && currFocusPath[1] === 'markDefs'

      if (isAnnotationPath) return

      // console.time('Update the FormBuilder focusPath')

      const {anchor, focus} = sel
      const isCollapsed = isEqual(focus.path, anchor.path) && focus.offset === anchor.offset
      ...
      if (isCollapsed && !isEqual(currFocusPath, focus.path)) {
        onFocus(focus.path)
      }

      // console.timeEnd('Update the FormBuilder focusPath')
    }, 250)

    updateFocusPathRef.current = updateFocusPath

    return () => updateFocusPath.cancel()
  }, [onFocus])

  // Trigger debounced `updateFocusPath` callback
  useEffect(() => {
    const updateFocusPath = updateFocusPathRef.current
    if (!updateFocusPath) return
    updateFocusPath(selection)
  }, [selection])
